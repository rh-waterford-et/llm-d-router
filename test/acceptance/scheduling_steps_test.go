...
func endpointsWithQueueDepths3(ctx context.Context, count, q1, q2, q3 int) (context.Context, error) {
	...
	endpoints := []fwksched.Endpoint{
		makeEndpoint("pod1", q1),
		makeEndpoint("pod2", q2),
		makeEndpoint("pod3", q3),
	}
...
func nEndpoints(ctx context.Context, count int) (context.Context, error) {
	...
	for i := range count {
		endpoints = append(endpoints, makeEndpoint(fmt.Sprintf("pod%d", i+1), i*2))
	}
	return withValue(ctx, keyEndpoints, endpoints), nil
}
...
func endpointsThatDoNotMatch(ctx context.Context, _ int) (context.Context, error) {
	// all candidates were eliminated by the filter; scheduler sees an empty list
	return withValue(ctx, keyEndpoints, []fwksched.Endpoint{}), nil
}
...
func endpointsWhereSomeMatch(ctx context.Context, total, matching int) (context.Context, error) {
	...
	for i := range total {
		...
		endpoints = append(endpoints, makeEndpoint(fmt.Sprintf("pod%d", i+1), qd))
	}
	return withValue(ctx, keyEndpoints, endpoints), nil
}
...
func makeEndpoint(name string, queueDepth int) fwksched.Endpoint {
